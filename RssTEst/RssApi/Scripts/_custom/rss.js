...
//    var dateformatee = (d <= 9 ? '0' + d : d) + "/" + (M <= 9 ? '0' + M : M) + "/" + date.getFullYear() + " - " + (h <= 9 ? '0' + h : h) + ":" + (m <= 9 ? '0' + m : m) + ":" + (s <= 9 ? '0' + s : s);
    ...
    return '<div class="row"><div class="col-lg-12">' + dateformatee + ' - <img  height="24" width="24" src="http://localhost:10793/api/Favicon/geticobysource/' + entree.SourceId + '" class="source" id="' + entree.SourceId + '"/><a href="' + entree.Link + '">' + entree.Titre + '</a></div></div>';
}